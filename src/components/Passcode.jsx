      ...
      if (passcode.length < 6) {
        ...
        if (newPasscode.length === 4) {
          ...
          if (enteredPasscode === CORRECT_PASSCODE) {
            ...
            setMessage('Kode sandi salah, petunjuk: tanggal Anniversary!, mau hts mulu kh :(')
            setTimeout(() => {
              setPasscode([])
              setMessage('')
            }, 4000)
  ...
  return (
    <div className="min-h-screen w-full bg-black/60 backdrop-blur-sm text-white flex items-center justify-center overflow-hidden">
        <div className="flex flex-col items-center max-w-full">
            {/* Title */}
            <h1 className="text-2xl font-light mb-8">Hayo berapa</h1>
            ...
            {/* Message */}
            {message && (
            <div className={`mb-4 -mt-9 text-sm font-bold ${message === 'Alloooooo adikkkk!! :)' ? 'text-green-500' : 'text-red-500'}`}>
                {message}
            </div>
            )}
